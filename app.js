...
const DB = {
  ...
  get:(key,fallback=null)=>{try{const v=localStorage.getItem("pn_"+key);return v?JSON.parse(v):fallback;}catch{return fallback;}},
  saveSession:(s)=>{const all=DB.get("sessions",[]);all.unshift({...s,id:Date.now()});DB.set("sessions",all.slice(0,500));},
  getTopicSessions:(sub,topic)=>DB.get("sessions",[]).filter(s=>s.subject===sub&&s.topic===topic),
  getGradeSessions:(grade)=>DB.get("sessions",[]).filter(s=>s.grade===grade),
  getAdaptiveLevel:(sub,topic,cur)=>{
    const ss=DB.getTopicSessions(sub,topic);
    if(ss.length<2)return cur;
    const avg=ss.slice(0,3).reduce((a,b)=>a+b.pct,0)/Math.min(ss.length,3);
    const lvls=["Beginner","Intermediate","Advanced"];
    const idx=lvls.indexOf(cur);
    if(avg>=80&&idx<2)return lvls[idx+1];
    if(avg<40&&idx>0)return lvls[idx-1];
    return cur;
  },
  getSmartSuggestions:(grade)=>{
    ...
    const map={};
    sessions.forEach(s=>{const k=s.subject+"||"+s.topic;if(!map[k])map[k]={subject:s.subject,topic:s.topic,sessions:[],level:s.level};map[k].sessions.push(s);});
    const sugg=[];
    Object.values(map).forEach(({subject,topic,sessions,level})=>{
      const avg=sessions.reduce((a,b)=>a+b.pct,0)/sessions.length;
      const lvls=["Beginner","Intermediate","Advanced"];
      const nextLvl=lvls[lvls.indexOf(level)+1];
      if(avg<50&&sessions.length>=2)sugg.push({type:"struggling",subject,topic,level,avg:Math.round(avg),message:`Needs more practice — avg ${Math.round(avg)}%`,action:"Practice again",priority:1});
      else if(avg>=80&&nextLvl)sugg.push({type:"ready_to_advance",subject,topic,level,nextLevel:nextLvl,avg:Math.round(avg),message:`Ready to level up — scoring ${Math.round(avg)}%!`,action:`Try ${nextLvl}`,priority:2});
      else if(avg>=60)sugg.push({type:"improving",subject,topic,level,avg:Math.round(avg),message:`Good progress at ${Math.round(avg)}% — keep going!`,action:"Continue",priority:3});
    });
    return sugg.sort((a,b)=>a.priority-b.priority).slice(0,6);
  },
  getWeekSummary:(grade)=>{
    const weekAgo=Date.now()-7*24*60*60*1000;
    const ss=DB.getGradeSessions(grade).filter(s=>s.id>weekAgo);
    if(!ss.length)return null;
    const avg=Math.round(ss.reduce((a,b)=>a+b.pct,0)/ss.length);
    const subs=[...new Set(ss.map(s=>s.subject))];
    const breakdown=subs.map(sub=>{const d=ss.filter(s=>s.subject===sub);return{subject:sub,count:d.length,avg:Math.round(d.reduce((a,b)=>a+b.pct,0)/d.length)};});
    return{sessions:ss.length,avgScore:avg,subjects:breakdown,needsWork:breakdown.sort((a,b)=>a.avg-b.avg)[0]?.subject,topSubject:breakdown.sort((a,b)=>b.avg-a.avg)[0]?.subject};
  }
};
...
const SAMPLE_WS = {
  "Mathematics||Fractions":{
    Beginner:{title:"Fractions — Beginner",grade:"Grade 2",subject:"Mathematics",level:"Beginner",time_suggested:"20 mins",total_marks:20,instructions:"Circle the correct answer or fill in the blanks.",mascot_tip:"A fraction means equal parts of a whole!",fun_fact:"Pizza is divided into fractions every day!",did_you_know:"The word fraction comes from Latin meaning broken.",sections:[
      {heading:"Identify the Fraction",section_label:"A",questions:[
        ...
        {id:3,type:"fill",prompt:"3 out of 4 equal parts = ___",answer:"3/4",marks:2},
        {id:4,type:"mcq",prompt:"Which is the biggest fraction?",options:["1/4","1/2","1/8","1/3"],answer:"1/2",marks:2},
        {id:5,type:"fill",prompt:"1/2 of 8 = ___",answer:"4",marks:2},
      ]},{heading:"Word Problems",section_label:"B",questions:[
        {id:6,type:"word_problem",prompt:"A pizza has 8 slices. You eat 3 slices. What fraction did you eat?",answer:"3/8",marks:4},
        {id:7,type:"fill",prompt:"1/4 of 12 = ___",answer:"3",marks:2},
        {id:8,type:"mcq",prompt:"Which fraction is equal to 2/4?",options:["1/3","1/2","2/3","3/4"],answer:"1/2",marks:2},
        {id:9,type:"fill",prompt:"1/2 of 20 = ___",answer:"10",marks:2},
        {id:10,type:"mcq",prompt:"What is 3/4 of 16?",options:["8","10","12","14"],answer:"12",marks:2},
      ]}]
    }
  },
  "English||Tenses – Present, Past, Future":{
    Beginner:{title:"Tenses — Beginner",grade:"Grade 2",subject:"English",level:"Beginner",time_suggested:"20 mins",total_marks:20,instructions:"Choose the correct tense or fill in the blank.",mascot_tip:"Present = now, Past = happened, Future = will happen!",fun_fact:"English has 12 tenses but we use just 3 main ones!",did_you_know:"The word tense comes from Latin meaning time.",sections:[
      {heading:"Identify the Tense",section_label:"A",questions:[
        {id:1,type:"mcq",prompt:"She plays cricket. — Which tense?",options:["Past","Present","Future","Perfect"],answer:"Present",marks:2},
        {id:2,type:"mcq",prompt:"They went to school. — Which tense?",options:["Present","Future","Past","Perfect"],answer:"Past",marks:2},
        {id:3,type:"mcq",prompt:"I will eat lunch. — Which tense?",options:["Past","Present","Future","None"],answer:"Future",marks:2},
        {id:4,type:"fill",prompt:"Change to past: She walks to school. She ___ to school.",answer:"walked",marks:2},
        {id:5,type:"fill",prompt:"Change to future: He reads a book. He ___ a book.",answer:"will read",marks:2},
        ...
        {id:9,type:"mcq",prompt:"Which is in present continuous tense?",options:["She ran fast.","She runs fast.","She is running fast.","She will run fast."],answer:"She is running fast.",marks:2},
        {id:10,type:"short",prompt:"Write one sentence in Past tense about your school.",answer:"Accept any correct past tense sentence.",marks:2},
      ]}]
    }
  },
  "Science/EVS||Plants – Parts & Functions":{
    Beginner:{title:"Plants — Beginner",grade:"Grade 2",subject:"Science/EVS",level:"Beginner",time_suggested:"20 mins",total_marks:20,instructions:"Answer all questions based on what you know about plants.",mascot_tip:"Plants make their own food using sunlight!",fun_fact:"The world tallest tree is 115 metres tall!",did_you_know:"Plants produce the oxygen we breathe.",sections:[
      {heading:"Parts of a Plant",section_label:"A",questions:[
        {id:1,type:"mcq",prompt:"Which part takes in water from the soil?",options:["Leaf","Flower","Root","Stem"],answer:"Root",marks:2},
      ...
      ]},{heading:"True or False",section_label:"B",questions:[
        {id:6,type:"mcq",prompt:"Roots always grow upward. True or False?",options:["True","False"],answer:"False",marks:2},
        {id:7,type:"short",prompt:"Name two plants we eat the roots of.",answer:"Carrot, radish, beetroot (any two)",marks:2},
        {id:8,type:"short",prompt:"Name two plants we eat the leaves of.",answer:"Spinach, cabbage, coriander (any two)",marks:2},
...
const getSampleWs=(subject,topic,level)=>{const key=subject+"||"+topic;return SAMPLE_WS[key]?.[level]||null;};
...
function App() {
  ...
  const buildChatGreeting = () => {
    const summary = grade ? DB.getWeekSummary(grade) : null;
    if(summary && summary.sessions > 0) {
      const needsWork = summary.needsWork ? ` ${summary.needsWork} needs more practice.` : "";
      return [{role:"assistant",content:`👋 Welcome back! 📊 This week: ${summary.sessions} worksheets, avg ${summary.avgScore}%.${needsWork}\n\nWhat would you like to work on today?`,chips:["Show AI suggestions","Build schedule","Add a topic"]}];
    }
    return [{role:"assistant",content:"👋 Hi! I\'m your PracticeNest worksheet assistant!\n\nI remember your child\'s progress and suggest the right level automatically!\n\n💡 Try: \'Maths fractions Grade 2\' or \'English tenses Grade 3 Advanced\'",chips:["Grade 2 Maths","Grade 3 English","Hindi topics","Show AI suggestions"]}];
  };
  ...
  const saveAndAdapt = (subject, topic, level, pct, timeSpent) => {
    ...
    setTopicMastery(prev=>{
      ...
      const old=prev[key]||{attempts:0,avgPct:0,bestPct:0,level};
      const n=old.attempts+1;
      return{...prev,[key]:{attempts:n,avgPct:Math.round((old.avgPct*old.attempts+pct)/n),bestPct:Math.max(old.bestPct,pct),lastPct:pct,level,timeSpent}};
    });
    const newLevel=DB.getAdaptiveLevel(subject,topic,level);
    if(newLevel!==level){
      setAdaptiveLevel(newLevel);
      const up=["Beginner","Intermediate","Advanced"].indexOf(newLevel)>["Beginner","Intermediate","Advanced"].indexOf(level);
      setNotifications(n=>[{id:Date.now(),read:false,icon:up?"🚀":"💪",text:up?`Great work on ${topic}! Moving up to ${newLevel} 🎉`:`Building confidence on ${topic} with ${newLevel} level`,time:"Just now"},...n.slice(0,9)]);
    }
  };
  ...
  const submitWs = ()=>{
    ...
    setSubmitted(true);
    const tSpent=wsStartTime?Math.round((Date.now()-wsStartTime)/1000):0;
    setWsTimeSpent(tSpent);
    saveAndAdapt(activeWs.subject,activeWs.topic,activeWs.level,pct,tSpent);
  };
  ...
  const renderSmartSuggestions = () => {
    ...
    if(!sugg.length) return (
      <div style={{textAlign:"center",padding:"20px 0",color:"#ccc"}}>
        ...
        <div style={{fontSize:13}}>Complete a few worksheets to get smart AI suggestions!</div>
      </div>
    );
    const colors={struggling:"#E74C3C",ready_to_advance:"#00B894",improving:"#6C63FF"};
    const icons={struggling:"⚠️",ready_to_advance:"🚀",improving:"📈"};
    return (
      <div>
        {sugg.map((s,i)=>{
          ...
          return(
            <div key={i} className="card" style={{padding:"12px 14px",marginBottom:8,borderLeft:`4px solid ${color}`}}>
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",gap:8}}>
                <div style={{flex:1}}>
                  <div style={{fontSize:11,color,fontWeight:800,marginBottom:2}}>{icons[s.type]} {SUBJECTS[s.subject]?.icon} {s.subject}</div>
                  <div style={{fontWeight:800,fontSize:13,color:"#333",marginBottom:2}}>{s.topic}</div>
                  <div style={{fontSize:12,color:"#666"}}>{s.message}</div>
                </div>
                <button className="btn" onClick={()=>openWorksheetWithIntro(s.subject,s.topic,s.nextLevel||s.level)}
          ...
          {dashTab==="suggestions"&&(
            <div>
              <div className="card" style={{padding:"12px 14px",marginBottom:10,borderLeft:"4px solid #6C63FF",background:"#F5F3FF"}}>
                ...
                <div style={{fontSize:12,color:"#888"}}>Based on your child actual performance data</div>
              </div>
