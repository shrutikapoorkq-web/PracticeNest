...
async function callClaude(system, user, maxT=3000) {
  ...
  if(!apiKey)throw new Error("No API key. Tap ⚙️ Settings to add your Anthropic API key.");
...
function App() {
  ...
  const CHAT_INIT = [{role:"assistant",content:"👋 Hi! I'm your PracticeNest worksheet builder. Tell me what you need and I'll collect everything before building!\n\n💡 Try: 'Maths fractions' or 'Grade 2 English tenses Advanced'",chips:["Build Daily Schedule","Build Weekly Schedule","Grade 2 Maths","Grade 3 English","Hindi Topics"]}];
...
I'll set it up for a **${sType}** schedule — you can add topics day by day and I'll build your worksheet plan.`,chips:[]}
  ...
  if(screen==="api-setup") return (
    <div style={{fontFamily:"'Nunito',sans-serif",background:"#F0EFFE",display:"flex",alignItems:"center",justifyContent:"center",minHeight:"100vh"}}>
      <style>{CSS}</style>
      <div className="card fadeUp" style={{padding:32,maxWidth:400,width:"100%",textAlign:"center",margin:16}}>
        <img src={window.LOGO_SRC||""} style={{width:72,height:72,objectFit:"contain",marginBottom:12}}/>
        <div style={{fontFamily:"'Fredoka One'",fontSize:22,color:"#333",marginBottom:6}}>PracticeNest Setup</div>
        <p style={{color:"#888",fontSize:13,marginBottom:20}}>Enter your Anthropic API key. Stored only in this browser.</p>
        <input id="api-key-inp" type="password" style={{...inp,marginBottom:12}} placeholder="sk-ant-..."/>
        <button className="btn" onClick={()=>{
          const k=document.getElementById("api-key-inp").value.trim();
          if(!k.startsWith("sk-")){alert("Enter a valid key starting with sk-");return;}
          localStorage.setItem("pn_api_key",k);go("splash");
        ...
        <div style={{fontSize:11,color:"#aaa"}}>Get a free key at console.anthropic.com</div>
      </div>
    </div>
  );
...
if(screen==="splash") return (
    <div style={{fontFamily:"'Nunito',sans-serif",background:"linear-gradient(135deg,#6C63FF,#FF6B6B)",minHeight:"100vh",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",gap:24,padding:24}}>
      ...
      <div className="pop" style={{textAlign:"center",color:"#fff"}}>
        <img src={window.LOGO_SRC||""} alt="PracticeNest" style={{width:130,height:130,objectFit:"contain",filter:"drop-shadow(0 4px 20px rgba(0,0,0,.3))"}} />
